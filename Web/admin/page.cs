...
namespace web
{
    public class page : web.BasePage
    {
        ...
        internal static string enumlist<T>(IPageLang lang, string propertyName, bool ignoreResourceMissing, params T[] items) where T : struct
        {
            ...
            {
                string k1 = typeof(T).Name;
                string k2 = t.ToString();
                string txt = Lang.GetLang(lang, string.Format("{0}.{1}", k1, k2)); //= lang[key];
                if (string.IsNullOrEmpty(txt))
                    txt = Lang.GetLang(k1, k2);
                if (string.IsNullOrEmpty(txt))
    ...
    public abstract class webcontrol : web.BaseWebControl
    {
    }
}

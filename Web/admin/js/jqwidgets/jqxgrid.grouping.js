...
(function(a) {
    a.jqx.dataview.grouping = function() {
        this.loadgrouprecords = function(E, H, s, F, I, f, R, G, x) {
            var M = E;
            var j = this;
            var Q = new Array();
            for (var n = 0; n < j.groups.length; n++) {
                Q[n] = j.generatekey()
            }
            var K = new Array();
            var b = 0;
            ...
            var O = this.sortdata != null;
            var S = O ? this.sortdata : this.records;
            if (this.pageable) {
                var A = Object.prototype.toString;
                var l = this.groups[0];
                Object.prototype.toString = (typeof l == "function") ? l : function() {
                    return this[l]
                };
                if (O) {
                    var c = this.sortfield;
                    Object.prototype.toString = (typeof l == "function") ? l : function() {
            ...
            for (var d = H; d < s; d++) {
                ...
                if (!O) {
                    ...
                    y = a.extend({}, S[d].value)
                }
                id = y[j.uniqueId];
                if (I >= G || id != R[I][j.uniqueId] || (f && f[id])) {
                    x[x.length] = I
                }
                var L = new Array();
                var w = 0;
                ...
                for (var N = 0; N < L.length; N++) {
                    ...
                    if (k == null) {
                        ...
                        K[b++] = k;
                        k.uniqueid = j.generatekey();
                        j.loadedgroupsByKey[p] = k
                    } else {
                        var o = {
                            group: p,
                            subItems: new Array(),
                        ...
                        j.loadedgroupsByKey[k.uniqueid + "_" + p] = o;
                        o.uniqueid = j.generatekey();
                        k.subGroups[k.subGroups.length++] = o;
                        k = o
                    }
                    B[m] = k
                }
                if (k != null) {
                    if (undefined == y.uid) {
                        y.uid = this.getid(this.source.id, y, P)
                    }
                    if (!O) {
                        y.boundindex = P;
                        this.recordsbyid["id" + y.uid] = S[d]
                    } else {
                        y.boundindex = S[d].index;
                        this.recordsbyid["id" + y.uid] = S[d].value
                    }
                    this.bounditems[y.boundindex] = y;
                    this.sortedgroups[P] = y;
                    y.uniqueid = j.generatekey();
                    y.parentItem = k;
                    y.level = k.level + 1;
                    k.subItems[k.subItems.length++] = y
                } else {
                    if (undefined == y.uid) {
                        y.uid = this.getid(this.source.id, y, P)
                    }
            ...
            var h = function(q, T, U) {
                for (var i = 0; i < T.subItems.length; i++) {
                    T.subItems[i].visibleindex = E + U;
                    q.rows[U] = T.subItems[i];
                    q.loadedrecords[U] = T.subItems[i];
                    U++
                }
                return U
            };
            var r = function(i, U, V) {
                var q = function(Y) {
                    if (i.aggregates == true) {
                        ...
                        if (W != null) {
                            X.level = W.level;
                            X.visibleindex = E + V;
                            X.uniqueid = i.generatekey();
                            i.rows[V] = X;
                            i.loadedrecords[V++] = X;
                            X.totalsrow = true;
                            if (Y.subItems.length > 0) {
                                W = Y.subItems[Y.subItems.length - 1];
                                X.parentItem = W.parentItem;
                                if (X.parentItem.subItems) {
                                    X.parentItem.subItems[X.parentItem.subItems.length] = X
                                }
                            } else {
                                if (Y.subGroups.length > 0) {
                                    W = Y.subGroups[Y.subGroups.length - 1];
                                    X.level = W.level;
                                    X.parentItem = Y;
                                    Y.subGroups[Y.subGroups.length] = X
                                }
                            }
                        }
                    }
                };
                for (subGroup in U.subGroups) {
                    var T = U.subGroups[subGroup];
                    if (T.subGroups) {
                        i.loadedgroups[i.loadedgroups.length] = T;
                        T.visibleindex = E + V;
                        i.rows[V] = T;
                        i.loadedrecords[V] = T;
                        V++;
            ...
            for (var P = 0; P < z; P++) {
                var t = K[P];
                this.loadedrootgroups[P] = t;
                this.loadedgroups[this.loadedgroups.length] = t;
                t.visibleindex = E + M;
                this.rows[M] = t;
                this.loadedrecords[M] = t;
                M++;
                M = r(this, t, M)
            }
            return M
    ...
    a.extend(a.jqx._jqxGrid.prototype, {
        ...
        _rendergroup: function(o, w, b, u, j, A, t) {
            ...
            var E = this._findgroupstate(b.uniqueid);
            if (b.bounddata.subGroups.length > 0 || b.bounddata.subItems.length > 0) {
                var i = this.rtl ? "-rtl" : "";
                var B = this.toThemeProperty("jqx-icon-arrow-right");
                if (i) {
                    B = this.toThemeProperty("jqx-icon-arrow-left")
                }
                if (E) {
                    c.className += " " + this.toThemeProperty("jqx-grid-group-expand" + i) + " " + this.toThemeProperty("jqx-icon-arrow-down")
                } else {
                    c.className += " " + this.toThemeProperty("jqx-grid-group-collapse" + i) + " " + B
                }
            }
            ...
            var l = F.cells[g];
            var z = 2;
            while (l != undefined && l.style.display == "none" && z < F.cells.length - 1) {
                l = F.cells[g + z - 1];
                z++
            }
            var f = a(l);
            ...
            if (this.groupsrenderer != null) {
                ...
                var p = this.groupsrenderer(s + ": " + b.group, b.group, E, h);
                if (p) {
                    l.innerHTML = p
                } else {
            ...
            if (this.rtl) {
                ...
                var C = this.vScrollBar.css("visibility") == "hidden" ? 0 : this.scrollbarsize + 6;
                var q = this.rowdetails && this.showrowdetailscolumn ? (2 + b.level) * d : (1 + b.level) * d;
                l.style.width = t + parseInt(r) - q - C + "px";
                f.addClass(this.toThemeProperty("jqx-rtl"));

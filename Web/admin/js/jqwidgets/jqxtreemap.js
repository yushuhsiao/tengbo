    ...
    c.extend(c.jqx._jqxTreeMap.prototype, {
        defineInstance: function() {
            var e = {
                ...
                height: 600,
                renderCallbacks: {},
                legendScaleCallback: function(f) {
                    return f
                },
                hoverEnabled: false,
        ...
        _layout: function(f, l) {
            ...
            var n = this._getValues(f.children),
                g = f.area.offset(),
                h = l(n, f.area.width(), f.area.height() - e, 0, e),
                m;
            for (var j = 0; j < f.children.length; j += 1) {
                var k = f.children[j];
                this._layoutRect(h[j], k);
                this._layoutArea(k, k.area);
                this._layout(k, l)
            }
            if (this.showLegend) {
                c(".jqx-treemap-legend").remove();
                this._renderLegend()
            }
        },
        _layoutRect: function(i, f) {
            ...
            if (typeof this.renderCallbacks[f.label] === "function") {
                ...
                var e = j.width() - 2;
                j.find(".jqx-treemap-label:first").css("max-width", e)
            }
        },

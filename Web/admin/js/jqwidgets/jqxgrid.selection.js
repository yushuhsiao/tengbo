...
(function(a) {
    a.extend(a.jqx._jqxGrid.prototype, {
        ...
        copyselection: function() {
            ...
            var l = m.getselectedcells();
            if (this.selectionmode.indexOf("cell") == -1) {
                l = this._getcellsforcopypaste()
            }
            if (l != null && l.length > 0) {
                ...
                for (var e = 0; e < l.length; e++) {
                    var h = l[e];
                    var b = m.getcolumn(h.datafield);
                    if (b != null) {
                        var j = m.getcelltext(h.rowindex, h.datafield);
                        var d = this.getrowdisplayindex(h.rowindex);
                        if (!this.clipboardselection[d]) {
                            this.clipboardselection[d] = {}
                        }
                        this.clipboardselection[d][h.datafield] = j;
                        n = Math.min(n, d);
                        k = Math.max(k, d)
                    }
                }
                for (var c = n; c <= k; c++) {
                    ...
                    if (this.clipboardselection[c] != undefined) {
                        a.each(this.clipboardselection[c], function(i, o) {
                            if (f > 0) {
                                g += "\t"
                            }
                            var p = o;
                            ...
                            m._clipboardselection[m._clipboardselection.length - 1][f] = p;
                            f++;
                            g += p
                        })
                    }
            ...
            this.clipboardselectedtext = g;
            return g
        },
        pasteselection: function() {
            var q = this.getselectedcells();
            if (this.selectionmode.indexOf("cell") == -1) {
                q = this._getcellsforcopypaste()
            }
            if (q != null && q.length > 0) {
                var p = q[0].rowindex;
                var i = this.getrowdisplayindex(p);
                var g = q[0].datafield;
                var m = this._getcolumnindex(g);
                var l = 0;
                this.selectedrowindexes = new Array();
                this.selectedcells = new Array();
                ...
                for (var r = 0; r < this._clipboardselection.length; r++) {
                    for (var k = 0; k < this._clipboardselection[r].length; k++) {
                        var h = this.getcolumnat(m + k);
                        if (!h) {
                            continue
                        }
                        var f = this.getrowboundindex(i + r);
                        var n = this.getcell(f, h.datafield);
                        var d = null;
                        d = this._clipboardselection[r][k];
                        if (d != null) {
                            if (h.cellsformat) {
                            ...
                            this.setcellvalue(f, n.column, d, false);
                            this._raiseEvent(18, {
                                rowindex: f,
                                datafield: n.datafield,
                                oldvalue: this.getcellvalue(n.rowindex, n.datafield),
                                value: d
                            });
                            this._applycellselection(f, n.datafield, true, false)
                        }
                    }
                }
                this.dataview.updateview();

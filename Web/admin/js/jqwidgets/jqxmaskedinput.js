    ...
    a.extend(a.jqx._jqxMaskedInput.prototype, {
        ...
        render: function() {
            ...
            if (e.element.nodeName.toLowerCase() == "div") {
                ...
                e.maskbox.attr("autocorrect", "off");
                e.maskbox.attr("autocapitalize", "off");
                e.maskbox.attr("spellcheck", false)
            }
            e.maskbox.addClass(e.toThemeProperty("jqx-reset"));
            ...
            if (e.rtl) {
                e.maskbox.addClass(e.toThemeProperty("jqx-rtl"))
            }
            var d = this;
            ...
            if (e.disabled) {
                e.maskbox.addClass(e.toThemeProperty("jqx-input-disabled"));
                e.maskbox.attr("disabled", true);
        ...
        _exitSimpleInputMode: function(b, n, h, d) {
            if (n == undefined) {
                n = b.data
            }
            if (n == null) {
            ...
            if (h == undefined) {
                ...
                if (o.left >= e && o.left <= e + c) {
                    if (o.top >= g && o.top <= g + l) {
                        return
                    }
                }
            }
            ...
            var k = a.data(n.maskbox, "simpleInputMode");
            if (k == null) {
                return
            }
            var j = n.maskbox[0].value;
            n.val(j);
            a.data(n.maskbox, "simpleInputMode", null);
            return false
        },
        _getString: function() {
            var c = "";
        ...
        _pasteSelectedText: function() {
            ...
            var f = this;
            var b = function(k) {
                if (k != f.selectedText && k.length > 0) {
                    f.selectedText = k;
                    if (f.selectedText == null || f.selectedText == undefined) {
                        return
                    }
                }
                if (j.start >= 0 || j.length > 0) {
                    for (i = j.start; i < f.items.length; i++) {
                        if (f.items[i].canEdit) {
                            if (c < f.selectedText.length) {
                                f.items[i].character = f.selectedText[c];
                                c++;
                                h = 1 + i
                            }
                        }
        ...
        _handleKeyDown: function(k, o) {
            ...
            if ((d && o == 120) || (d && o == 88)) {
                this.selectedText = this._saveSelectedText(k);
                this._deleteSelectedText(k);
            ...
            if ((d && o == 118) || (d && o == 86) || (k.shiftKey && o == 45)) {
            ...
            if (n.start >= 0 && n.start < this.items.length) {
                var g = String.fromCharCode(o);
                var p = this.items[n.start]
            }
            if (o == 8) {
                if (n.length == 0) {
                    for (j = this.items.length - 1; j >= 0; j--) {
                        if (this.items[j].canEdit && j < n.end && this.items[j].character != this.promptChar) {
                            this._setSelection(j, j + 1);
                            break
                        }
                    }
                }
                n = this._selection();
                var h = this._deleteSelectedText();
                if (n.start > 0 || n.length > 0) {
                    if (n.start <= this.items.length) {
                        if (h) {
                            this._setSelectionStart(n.start)
                        } else {
                            this._setSelectionStart(n.start - 1)
                        }
                    }
                }
                return false
            ...
            if (o == 190) {
                var c = n.start;
                for (var j = c; j < this.items.length; j++) {
                    if (this.items[j].character == ".") {
            ...
            if (o == 46) {
                if (n.length == 0) {
                    for (var j = 0; j < this.items.length; j++) {
                        if (this.items[j].canEdit && j >= n.start && this.items[j].character != this.promptChar) {
                            this._setSelection(j, j + 1);
                            break
                        }
                    }
                    ...
                    if (n.start < this.items.length) {
                        if (n.length <= 1) {
                            if (b.end != n.end) {
                                ...
                                this._setSelectionStart(n.end + 1)
                            }
                        } else {
                            this._setSelectionStart(n.start)
                        }

...
// Form Functions
	createEl : function(eltype,options,vl,autowidth, ajaxso, rwd) {
		var elem = "", $t = this;
		...
		switch (eltype)
		{
			...
			case "select" :
				...
				if(options.dataUrl !== undefined) {
				...
				} else if(options.value || options.value_func) {
					var i;
					...
					if ($.isFunction(options.value_func)) options.value = options.value_func.call(elem, rwd);
			...
			case "custom" :
				...
				try {
					if($.isFunction(options.custom_element)) {
						var celm = options.custom_element.call($t,vl,options, elem);
						if(celm) {
							//celm = $(celm).addClass("customelement").attr({id:options.id,name:options.name});
							$(elem).addClass("customelement").attr({ id: options.id, name: options.name }).empty().append(celm);
						} else {
							throw "e2";
						}
					} else {

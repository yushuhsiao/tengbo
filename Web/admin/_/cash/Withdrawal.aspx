    ...
    <script type="text/javascript">
        ...
        $(document).ready(function (ind, rowid) {
            ...
            $table = $('#table1').jqGrid_init({
                ...
                postData: { hist: hist },
                SelectCommand: function (postData) { return { WithdrawalSelect: postData } },
                UpdateCommand: function (postData) { return { WithdrawalUpdate: postData } },
                InsertCommand: function (postData) { return { WithdrawalInsert: postData } },

                // 收款卡號(客戶), 出款卡號(公司), 交易資訊(帳務), 交易資訊(網銀查帳)
                ...
                colModel: [
                    ...
                    { name: 'State     ', label: '<%=lang["colState     "]%>', width: 080, sorttype: 'text    ', editable: false, formatter: 'select', edittype: 'select', editoptions: { <%=enumlist<WithdrawalState>("value")%> } },
                    ...
                    { name: 'Commit    ', label: '<%=lang["colCommit    "]%>', width: 080, sorttype: 'text    ', editable: !hist, align: 'left', hidden: hist, formatter: $.empty, edittype: 'checkboxs', editoptions: { multiple: false,<%=enumlist<CommitOrRollback>("value")%> } },
                  //{ name: 'MemberID  ', label: '<%=lang["colMemberID  "]%>', width: 050, sorttype: 'int     ', editable: false, hidden: true },
                    ...
                    { name: 'AmountW   ', label: '<%=lang["colAmountW   "]%>', colType: 'Money', editable: false, editonce: false },
...
<asp:Content ID="Content2" ContentPlaceHolderID="body" runat="Server">
    <div id="nav1">
        <div class="">
            ...
            <a id="btnSwitch" href="Withdrawal.aspx?t=h"><%=lang["btnSwitch2"]%></a>
            <% } else { %>
            <a id="btnSwitch" href="Withdrawal.aspx"><%=lang["btnSwitch1"]%></a>
            <% } %>
        </div>
    </div>
    <table id="table1">

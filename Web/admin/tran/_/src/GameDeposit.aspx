...
<asp:Content ContentPlaceHolderID="head" runat="Server">
    <script type="text/javascript">
        ...
        var logType = '<%=(int)LogType.GameDeposit%>';

        $(document).ready(function (ind, rowid) {

            $table = $('#table1').jqGrid_init({
                pager: true, loadonce: !hist, nav1: '#nav1', nav2: '#nav2', postData: { IsDeposit: true, IsHist: hist },
                SelectCommand: function (postData) { postData.LogType = logType; return { GameTranSelect: postData } },<%if(!hist){%>
...
<%if(!hist){%>          { name: 'Action         ', label: '<%=lang["colAction       "]%>', colType: 'Buttons' },
                        { name: 'Accept         ', label: '<%=lang["colAccept       "]%>', width: 060, sorttype: 'text    ', editable: true, align: 'left', formatter: $.empty, edittype: 'checkboxs', editoptions: { multiple: false, value: { 1: '<%=lang["op_Accept"]%>' } } },

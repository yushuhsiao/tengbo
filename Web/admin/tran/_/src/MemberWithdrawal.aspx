    ...
    <script type="text/javascript">
        ...
        $(document).ready(function (ind, rowid) {
            ...
            $table = $('#table1').jqGrid_init({
                useDefValues: true, pager: true, loadonce: !hist, nav1: '#nav1', nav2: '#nav2', postData: { IsDeposit: false, IsHist: hist },
...
<%if(!hist){%>          { name: 'Action         ', label: '<%=lang["colAction       "]%>', colType: 'Buttons' },
                        ...
                        { name: 'LogType        ', label: '<%=lang["colLogType      "]%>', width: 080, sorttype: 'text', editable: true, editonce: true, edittype: 'select', editoptions: { <%=enumlist<LogType>("value", true, BU.LogType.Withdrawal)%> }, formatter: 'select', formatoptions: { <%=enumlist<LogType>("value")%> } },

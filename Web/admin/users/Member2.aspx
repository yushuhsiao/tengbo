...
<asp:Content ContentPlaceHolderID="head" runat="server">
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            var _postdata = { UserType: '<%=(int)this.UserType%>', UserID: '<%=userID%>' };
            function onSelectRow(rowid, status, e) { /*$($(this).getInd(rowid, true)).removeClass('ui-state-highlight');*/ };
            function UpdateCommand(postData) { return { UserGameUpdate: $.extend(true, postData, _postdata) }; };
            ...
            function getBalance(rowid, iCol, cellcontent, e) {
                ...
                if ($cell.hasClass('gamebalance-load')) {
                    ...
                    if (!$cell.hasClass('gamebalance-loading')) {
                        ...
                        $.invoke_api({ UserGameBalance: { GameID: rowid, UserID: '<%=userID%>', UserType: '<%=(int)BU.UserType.Member%>' } }, {
                            success: function (data, textStatus, jqXHR) {
                                if (data.Status == 1) {
                                    sendMessage('UserGameRowData', data.row);
            ...
            function gameTran(cell, cm, cellIndex) {
                ...
                $('.d, .w', cell).button().removeClass('ui-state-default').click(function () {
                    ...
                    var postData = {
                        ...
                        GameID: rowid,
                        UserID: '<%=userID%>',
                        Amount: parseFloat($.trim($txt.val()))
                    };
                    var obj = {};
                    obj[$(this).hasClass('d') ? 'MemberGameDeposit_Update' : 'MemberGameWithdrawal_Update'] = postData;
                    ...
                    if ($txt.prop('disabled') == false) {
                        ...
                        $.invoke_api(obj, {
                            success: function (data, textStatus, jqXHR) {
                                ...
                                if (data.row) {
                                    sendMessage('MemberRowData', { ID: data.row.UserID, Balance: data.row.UserBalance });
                                    if (data.row.UserGameRow)
                                        sendMessage('UserGameRowData', data.row.UserGameRow);
                                }
                            },
                            error: function (xhr, ajaxOptions, thrownError) {

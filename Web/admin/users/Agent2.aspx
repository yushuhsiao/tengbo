...
<asp:Content ContentPlaceHolderID="head" runat="server">
    <script type="text/javascript">
        ...
        var recvMessage = {
            AgentGameRowData: function (data) {
            ...
            function getBalance(rowid, iCol, cellcontent, e) {
                ...
                if ($cell.hasClass('gamebalance-load')) {
                    ...
                    if (!$cell.hasClass('gamebalance-loading')) {
                        ...
                        $.invoke_api({ UserGameBalance: { GameID: rowid, UserID: '<%=userID%>', UserType: '<%=(int)BU.UserType.Agent%>' } }, {
                ...
                $('.d, .w', cell).button().removeClass('ui-state-default').click(function () {
                    ...
                    if ($txt.prop('disabled') == false) {
                        ...
                        $.invoke_api({ GameTranInsert: postData }, {
                            success: function (data, textStatus, jqXHR) {
                                if (data.row) {
                                    sendMessage('AgentRowData', { ID: data.row.AgentID, Balance: data.row.Balance });
                                    if (data.row.AgentGameRow)
                                        sendMessage('AgentGameRowData', data.row.AgentGameRow);
                                }
                            },
                            error: function (xhr, ajaxOptions, thrownError) {

...
<script runat="server">
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        using (SqlCmd sqlcmd = DB.Open(DB.Name.Main, DB.Access.ReadWrite))
        {
            ...
            foreach (SqlDataReader r in sqlcmd.ExecuteReader2("select * from AgentBank nolock where AgentID={0}", this.agentID))
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            function RowResponse(res, rowid, data) {
                ...
                if (data.dst) sendMessage('AgentRowData', { ID: data.dst.AgentID, Balance: data.dst.Balance });
                //if (data.Agent) sendMessage('AgentUpdate', data.Agent);
                //if (data.Parent) sendMessage('AgentUpdate', data.Parent);
            }
            $table1 = $('#table1').jqGrid_init({
                ...
                InsertCommand: function (postData) { postData.AgentID = '<%=agentID%>'; return { AgentBankCardInsert: postData } },
                UpdateCommand: function (postData) { postData.AgentID = '<%=agentID%>'; return { AgentBankCardUpdate: postData } },
                DeleteCommand: function (postData) { postData.AgentID = '<%=agentID%>'; return { AgentBankCardDelete: postData } },
                colModel: [
            ...
            $table2 = $('#table2').jqGrid_init({
                ...
                onSelectRow: onSelectRow, RowResponse: RowResponse, UpdateCommand: function (postData) { postData.AgentID = '<%=agentID%>'; return { AgentTranInsert: postData } },
                colModel: [
            ...
            $table4 = $('#table4').jqGrid_init({
                ...
                onSelectRow: onSelectRow, RowResponse: RowResponse, UpdateCommand: function (postData) { postData.AgentID = '<%=agentID%>'; return { ParentTranA: postData } },
                colModel: [
...
<asp:Content ContentPlaceHolderID="body" runat="server">
    ...
    <table id="table2">
        <tr class="colModel">
            <td name="Action">
                <span property="action">
                    <div class="edithide" action="editRow" icon="ui-icon-plus"><%=lang["btnAdd"]%><%=lang["menu", "AgentTran Deposit"]%></div>
                    <div class="editshow" action="restoreRow" icon="ui-icon-cancel"><%=lang["Actions_Cancel"]%></div>
    ...
    <table id="table3">
        <tr class="colModel">
            <td name="Action">
                <span property="action">
                    <div class="edithide" action="editRow" icon="ui-icon-plus"><%=lang["btnAdd"]%><%=lang["menu", "AgentTran Withdrawal"]%></div>
                    <div class="editshow" action="restoreRow" icon="ui-icon-cancel"><%=lang["Actions_Cancel"]%></div>
                    <div class="editshow" action="saveRow" icon="ui-icon-check"><%=lang["Actions_OK"]%></div>
                </span>
            </td>
    ...
    <% if (User.Permissions[Permissions.Code.child_agent_balance]) { %>
    <hr />
    <table id="table4">
        <tr class="colModel">
            <td name="Action">

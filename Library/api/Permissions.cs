...
namespace BU
{
    public class Permissions
    {
        ...
        public int Load(SqlCmd sqlcmd, UserType userType, int corpID, byte groupID, int userID)
        {
            if (this.IsNull)
                return 0;
...
left join Permission2 b with(nolock) on a.ID=b.CodeID and b.UserType={0} and b.CorpID={1} and b.GroupID={2}
select a.*, isnull(c.Flag,0) as Flag from Permission1 a with(nolock)
left join Permission3 c with(nolock) on a.ID=c.CodeID and c.UserID={3}", (byte)userType, corpID, groupID, userID))
                {
